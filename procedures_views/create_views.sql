...
SELECT 
		...
		a.city 			as 'Cidade',
		d.timeDelivered as 'Data de Entrega'
FROM address a
...
SELECT 
		name			as 'Nome da Loja',
		customerGrade 	as 'Avaliação'
FROM store;
...
SELECT 
		COUNT(pk_pid)	as 'Número de Produtos',
		fk_brand		as 'Marca'
FROM product
GROUP BY fk_brand;
...
SELECT 
		o.pk_orderNumber	as 'Número do Pedido',
		o.paymentStatus 	as 'Estado do Pagamento',
        COALESCE('Cartão de Crédito', 'Cartão de Débito') as 'Método de Pagamento'
FROM orders o
...
SELECT 
		c.pk_orderNumber		as 'Pedido',
		SUM(c.quantity)			as 'Quantidade'
FROM contain c 
...
SELECT 
		...
		COUNT(o.pk_orderNumber)		as 'Total de Vendas'
FROM store s
...
SELECT 
		p.name			as 'Produto',
		AVG(c.grade)	as 'Avaliações Médias'
FROM product p 
...
SELECT 
		c.pk_cardNumber as	'Número do Cartão',
		b.expiryDate	as  'Data de Validade'
FROM creditCard  c
...
SELECT 
		SUM(p.pk_pid) as	'Número de Produtos',
		fk_brand	  as	'Marca'
FROM product 
GROUP BY fk_brand;
...
SELECT 
		s.name	as	'Nome da Loja',
		u.name	as	'Nome do Gerente/ Vendedor'
FROM store s
